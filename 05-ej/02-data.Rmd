...
Despite E.O. 12868, most rules do not address EJ. Figure \@ref(fig:agencies100) 

```{r ej-data, fig.cap = "Number of Proposed and Final Rules Addressing Environmental Justice.", out.height = NULL, out.width = "49%", fig.show = "hold"}
...
Even at the Environmetal Protection Agency (EPA), where most politices presumably are clearly related to "environmental" issues, a consistant minority of rules address EJ. Agencies almost never address EJ include agencies that make policy with clear EJ effects, including Housing and Urban Development (HUD), the Nuclear Regulatory Commission (NRC), and Office of Surface Mining (OSM). A majority of rules addressed EJ only in a few years at a few agencies that publish relativly few rules such as the Concil of Environmental Quality (CEQ), Federal Emergancy Managment Agency (FEMA), and several Transportation agencies (the Federal High Way Administration, Federal Transit Administration (FTA), Federal Railroad Administration (FRA)). Figure \@ref(fig:agencies100) shows the number of rulemaking projects at each agency over time by whether they ultimatly addressed EJ. 


```{r ej-data-agencies100, fig.cap = "Number of Proposed and Final Rules Addressing Environmental Justice", out.height = NULL, out.width = "49%", fig.show = "hold"}

knitr::include_graphics(here::here("Figs", "ej-data-agencies100.png") )
```
...
```{r ej-comments, fig.cap = "Number of Comments on Proposed and Final Rules that Did and Did Not Address Environmental Justice and Whether Comments Raised Environmental Justice Concerns", out.height = NULL, out.width = "49%", fig.show = "hold"}

knitr::include_graphics(here::here("Figs", "ej-comments.png") )
```
...
```{r ejcommentsbyrace, fig.cap = 'Estimated Racial Distribution from Census Surnames of Commenters raising "Environmental Justice" Concerns', out.height = NULL, out.width = "49%", fig.show = "hold"}
...
Where draft rules that did not address EJ, I measure whether added "environmental justice" between its draft and final publication. This is usually took the form of the addition of an "E.O.12898 section" to the preable of the final rule where the agency justifies its policy changes. The next most common addition came in the the agency's response to comments, explaining how the rule did not have disportate effects or that they were insignificant. Sometimes both occured, for example, the EPA responded to several commenters including Earthjustice, the Central Valley Air Quality Coalition, the Coalition for Clean Air, Central California Environmental Justice Network, and Central California Asthma Collaborative: "EPA agrees it is important to consider environmental justice in our actions and we briefly addressed environmental justice principles in our proposal." EPA had not, as the commenters noted, addressed environmental justice in the proposed rule, which approved California rules regulating particulate matter emissions from construction sites, unpaved roads, and disturbed soils in open and agricultural areas. EPA did add a fairly generic 12898 section to the final rule, but did not substantively change its position. As this example illustrates, agencies may add text addressing environmental justice that would in no way satisfy critics. This meassure merely indicates whether the agency engaged with the claims.

## Measure 2: Changing Text Addressing EJ in Final Rules

Where draft rules that did address EJ, I measure whether a rule changed *how* it discussed "environmental justice" between its draft and final publication.^[Occasionally, there is more than one version of a Proposed or Final rule on a rulemaking docket. Here I opt for an inclusive measure of change that counts change from *any* proposed to *any* final rule. This means that if the change occured between the first and second draft of a proposed rule, it is counted as a change. This best capures the concept of rule change. However, estimates are similar if we only count cases where change occured between *every* version of rule.] 
