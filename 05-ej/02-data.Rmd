...
```{r ej-comments, fig.cap = "Number of Proposed and Final Rules Addressing Environmental Justice", out.height = NULL, out.width = "49%", fig.show = "hold"}
...
## The Distributional Politics of Murcury Pollution
...
## Distributional Politics of Air Pollution
...
I use two simple indicators of policy change to model the effect of public comments on policy: *whether* a rule addresses EJ and *how* it addresses EJ. Both measures represent a fairly low bar, indicating whether the agency explicitly paid any attention at all to ej. This is appropriate given prior research shows little to no effect of public comments from advocacy groups as well as little attention to EJ in particular. 

## Adding Text Addressing EJ to Final Rules



## Changing Text Addressing EJ in Final Rules

Where draft rules that did address EJ, I measure whether a rule changed *how* it discussed "environmental justice" between its draft and final publication.^[Occasionally, there is more than one version of a Proposed or Final rule on a rulemaking docket. Here I opt for an inclusive measure of change that counts change from *any* proposed to *any* final rule. This means that if the change occured between the first and second draft of a proposed rule, it is counted as a change. This best capures the concept of rule change. However, estimates are similar if we only count cases where change occured between *every* version of rule.] I parse draft and final rules into sentences and identify sentences 

In many cases, much of the text of final rules remains exactly the same between draft and final versions. When an agency address EJ in the draft rule, often in a section about how it addressed E.O.12898, this section stays the same, meaning that the sentences exactly match between draft and final rules.^[An alternative approach would be to parse documents by section and assess whether E.O.12898 sections are identical. Parsing by sentences has two advantages: it is computationally faster, it avoids problems with section numbering and other frustrations with section matching, and (3) it captures attentoin to EJ outside of this section, especially in the response to comments section. If an agency is paying attention to EJ issues, sentence matching will be most likely to detect it. However, other measures, such as the percent of EJ sentences changed, the percent of words in a 12898 section that changed, or the change in topic proportions [@Judge-Lord2017] could be useful in future work.]
