...
```{r ej-descriptive, position = "hold", out.width="49%", fig.cap = "Descriptive", out.width = "49%", fig.subcap=""}
...
Models "2 Mass Only" and "4 Mass Only" are identical to Models 2 and 4, but subset to rules that recieved over 100 comments.
Models "2 EJ Only" and "4 EJ Only" are identical to Models 2 and 4, but subset to rules that received at least on comment raising EJ concerns instead of including the indicator. 
...
Table \@ref(tab:ej-appendix2) presents an alternative specification for the main models 2 and 4, as well as for the robustness checks in Table \@ref(tab:ej-appendix1). While we have strong reasons to belive that comments have decreasing maginal effect, and the logged number of comments the offers a parsamonious modeling strategy, the models in Table \@ref(tab:ej-appendix2) relax this assumption about the functional form by using quadradic terms instead. In these tables, "Total Comments" is measured in 1000s whereas "Unique EJ Comments" is not transformed. With this specification, most of the hypothesis cannot be assessed by the p-value on any one term. 
...
```{r ej-robust,  out.width="49%", position = "hold"}
...
```{r ej-robust-figs, position = "hold", out.width="49%"}
