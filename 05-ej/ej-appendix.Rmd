...
ejcomments %>% count(docket_id, number_of_comments_received, sort = T) %>% arrange(-number_of_comments_received)
```
...
allFR %>% 
  ...
  mutate(ej_comment = ifelse(ej_comment, "EJ Raised", "EJ Not Raised")) %>% 
  ggplot() + 
  aes(x = Year, y = n,
      fill = factor(ej_comment, levels = c("EJ Not Raised", "EJ Raised"))) +
  scale_fill_discrete(direction = -1) + 
  geom_col(alpha = .7) + 
...
ejFR_PR %>% 
  ...
  mutate(mean = mean(c(`TRUE`, `FALSE`)),
         EJ_fr = ifelse(ej_fr, "EJ Addressed", "EJ Not Addressed"),
...
predicted <- augment(m_PR,  
  ...
  left_join(ejFR_PR %>% count(ej_comment, name = "n_ej_comment") ) %>% 
  mutate(EJ_comment = ifelse(ej_comment, "EJ Raised", "EJ Not Raised") %>%             str_c(", N = ", n_ej_comment))
...
predicted %<>% 
  ...
  mutate(Agency = str_c(diff, "% increase at ", agency, ", N = ", n_agency),
         EJ_comment = ifelse(ej_comment, "EJ Raised", "EJ Not Raised") %>%             str_c(", N = ", n_ej_comment))
...
- [ ] Clarify distributive - Lowi

- [ ] Clarify that I'm not arguing against the dominance of well-resourced groups 

- [ ] More on ideas, discourse, framing in public policymaking (Stone and Schneider and Ingraham)



Potential outcomes (ideally have an example of each): 
