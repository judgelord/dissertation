...
```{r ej-data-agencies100, fig.height=4.5}
# ej-data-agencies100
...
```{r ej-m-PR-shareI, fig.width=6, fig.height=2.5}
# ej-m-PR-comments-share
...
predicted %>%
  ...
  geom_pointrange(aes(ymin = predicted - 1.96*std.error,
                  ymax = predicted + 1.96*std.error), alpha = .7 )  + 
  geom_text(check_overlap = T, color = "gray", hjust = "inward",
            y = .8,
            #aes(y = #ifelse(ej_comment, predicted, NA))
            ) + 
  geom_hline(yintercept = 0, linetype = 2) +
  #scale_y_continuous(breaks = c(0, .5,1)) +
    scale_color_viridis_d(begin = 0, end = .6, option = "plasma") +
...
values <- ejFRejPR %>% 
  tidyr::expand(comments =  c(1, 10,100,1000,10000),
         ...
         president = "Obama",
         agency = "EPA",
         agency_ej_share = .67) %>% mutate(ej_comments_unique = as.numeric(ej_comment))
...
ej_rates <- rules %>% 
  ...
  transmute(Agency = agency, Rules = n, `Mention EJ` = agency_ej_share %>% percent(1) ) 
