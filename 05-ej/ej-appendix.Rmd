...
rules %>% 
  ...
  count(year, ej_pr) %>% 
  pivot_wider(values_from = n, ej_pr, id_cols = year) %>% 
...
rules %>% 
  filter(document_type == "Proposed Rule") %>% 
  mutate(ej_pr = ifelse(ej_pr, "EJ Addressed", "EJ Not Addressed")) %>% 
  count(president, ej_pr) %>% 
  pivot_wider(values_from = n, ej_pr, id_cols = president) %>% 
  mutate(sum = `EJ Addressed` + `EJ Not Addressed`,
         percent = `EJ Addressed` / sum ) %>% 
  kablebox()
...
# pr over time
rules %>% 
  filter(document_type == "Proposed Rule", year > 2004) %>% 
  mutate(ej_pr = ifelse(ej_pr, "EJ Addressed", "EJ Not Addressed")) %>% 
  ggplot() + 
...
rules %>% 
  ...
  labs(y  = "All Proposed Rules",
       ...
       fill = "")+ # "EJ Addressed\nin Proposed Rule") + 
  scale_fill_viridis_d(option="plasma", begin = 0, end = .6, direction = 1) + 
    facet_wrap("president2", nrow = 1, scales = "free") + 
  theme(axis.text.x = element_text(angle = 45, vjust = .5),
        panel.grid.major.x = element_blank(),
        panel.border = element_blank())
...
# fr type over time
rules %>% 
  filter(document_type == "Rule", year > 2004) %>% 
  mutate(ej_fr = ifelse(ej_fr, "EJ Addressed", "EJ Not Addressed")) %>% 
  ggplot() + 
  aes(x = year, fill = ej_fr) +
  geom_bar(alpha = .7, position = "dodge") + 
    facet_wrap("president2",nrow = 1, scales = "free_x") + 
...
```{r ej-data-agencies}
share <- allFR %>%
  distinct(ej_fr, docket_id, agency) %>% 
  group_by(agency) %>% 
  summarise(agency_ej_share = sum(ej_fr)/ n() )  %>% 
...
```{r ej-data-agencies100, fig.height=6.5}
# ej-data-agencies100
...
predicted %>%
  ...
  geom_pointrange(aes(ymin = predicted - 1.96*std.error,
                  ymax = predicted + 1.96*std.error), alpha = .7 , position = position_dodge(width = 0.40)  )  + 
  geom_hline(yintercept = 0, linetype = 2) +
...
predicted %>%
  ...
  aes(x = agency_ej_share, 
      ...
      label = agency) + 
  geom_pointrange(aes(ymin = predicted - 1.96*std.error,
                  ymax = predicted + 1.96*std.error), alpha = .7 , 
  ...
  geom_text(check_overlap = T, 
            ...
            hjust = "left",
            #y = .7,
            aes(y = ifelse(ej_comment, predicted + 1.96*std.error + .05, NA))
            ) + 
...
coord_flip() +
  #facet_wrap("ej_comment", ncol = 1) +
  labs(y = 'Probability that "Environmental Justice"\nis Added to Final Rule', 
       x = 'Share of Rules Mentioning "Environmental Justice"',
       color = "",#color = '"Environmental Justice"\nRaised by Comments',
       shape = "",#shape = '"Environmental Justice"\nRaised by Comments',
       title = "Predicted Change in Final Rules")  +
  theme(panel.border  = element_blank(),
...
predicted <- predictions(m_PR_epa, newdata = values) 
...
predicted <- predictions(m_PR_mass, newdata = values) 
...
predicted %<>% 
  left_join(ejFR_PR %>% filter(comments > 99) %>%  count(ej_comment, name = "n_ej_comment") ) %>% 
  mutate(EJ_comment = ifelse(ej_comment, "Comments Address EJ", "No Comments Address EJ") %>%             str_c(", N = ", n_ej_comment))
...
predicted <- predictions(mejPR_epa, newdata = values) 
...
predicted <- predictions(mejPR_mass2, newdata = values) 
...
save(ejFR_PR, 
     ...
     mejPR_mass2,
     mejPR_agencyFE_ejcomment2,
     file = here::here("05-ej", "ej_models.Rdata") ) 
...
save(ej_rates, file = here::here("05-ej", "ej_rates.Rdata"))
```
