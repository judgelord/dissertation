...
### Subset where EJ was raised

(as well as Agency and President FE)

This is just a robustness check. The effect of an EJ comment should capture differences. 

```{r ej-m-PR-comments-agencyFE-ejcomment, fig.width=6, fig.height=2.5}
# ej-m-PR-comments-agencyFE

# agencies with variation on DV
# ejFR_PR %<>% group_by(agency) %>% mutate(dv_var = ej_fr %>% unique() %>% length()) %>% ungroup() %>% filter(dv_var > 1)

m_PR_agencyFE_ejcomment <- feglm (ej_fr ~ comments + 
                                    comments^2 +
                                    ej_comments_unique + 
                                    ej_comments_unique^2
                        | president + agency, 
...
modelsummary(m_PR_agencyFE_ejcomment, stars = T)
...
```{r ej-m-PR-ejcomments-agencyFE-ejcomment, fig.width=6, fig.height=2.5}
# ej-m-PR-ejcomments-agencyFE-ejcomment
