...
ejPR %>% 
  ...
  aes(x = year) +
  geom_bar(alpha = .7, position = "dodge") + 
  labs(y  = "Proposed Rules Addressing Environmental Justice",
       x = "Year") + 
  theme(axis.text.x = element_text(angle = 45, vjust = .5))
```
...
ejFR %>% 
  ...
  aes(x = year) +
  geom_bar(alpha = .7, position = "dodge") + 
  labs(y  = "Final Rules Addressing Environmental Justice",
       x = "Year") + 
  theme(axis.text.x = element_text(angle = 45, vjust = .5))
...
Subsetting to agencies that pubished at least one rule explicitly addressing EJ from 1992 through 2020 yields
 `r rules %>% distinct(docket_id, comments) %>% summarize(n = sum(comments)) %>% pull(n)` public comments on
 `r rules %>% distinct(docket_id) %>% nrow()` rulemaking dockets from
