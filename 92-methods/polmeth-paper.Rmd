...
Agency rules and regulations are where the rubber hits the road for both legislation and presidential agendas [@Yackee2009bush]. Ninety percent of U.S. law is now written by agencies rather than Congress [@West2013]. Legally binding regulations give specific meaning and force to legislation and presidential agendas. 
The real impact of most new statutes and executive orders is largely unknown until the ink is dry on the agency rules and regulations that implement them.
...
Before reviewing specific tools and methods, I briefly address the broader data management system that makes iterative hand-coding and computational text analysis feasible in a team setting. 
...
One advantage of real-time analysis was the ability to systematically check for irregularities in coders' work. For example, when new data were coded on sheets, scripts measured the variance in the commenter positions and demands described in Section \@ref(success) helped assess the coherence of hand-coded coalitions.


> TODO: MORE ON CODER CHECKS

Additionally, the new information provided by coders allowed constant updates of algorithms that auto-coded organizations and coalitions, as described in Section \@ref(processing). Updated auto-coding automatically populated the google sheets, reducing the remaining work for coders. 
...
In many cases, using text data requires that we first extract it from pdf. BLA BLA BLA
...
Through an iterative process of computational text analysis and hand-coding, I attribute comments to the organizations, campaigns, and broader coalitions that mobilized them. 
