...
comments_coded %>% 
  ...
  group_by(president, coalition_type, EJ_org) %>% 
  add_count() %>% 
  group_by(n, success, president, coalition_type, EJ_org) %>% 
  add_count(.drop = F) %>% 
  mutate(percent = (nn/n) ) %>% 
  ggplot() + 
  aes(x = success, fill = EJ_org, y = percent) +
  geom_col(alpha = .5, position = "dodge", width = 1) + 
...
dejpr %>% 
  ...
  geom_jitter() + 
  facet_grid(coalition_type ~ ej_org)
