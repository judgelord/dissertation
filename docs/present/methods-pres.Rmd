...
3. Humans and machines can iteratively **tag entities using regex tables** (e.g., identify organizations in documents)
4. Humans and machines can iteratively **group texts by key features** (e.g., identify lobbying coalitions by common policy demands)
...
```{r, out.width = c("84%", "16%"), fig.cap= 'Iteratively build regex tables. For example, the `legislators` package adds name varients (e.g., "AOC") to standard legislator names', fig.show='hold', fig.topcaption=TRUE}
