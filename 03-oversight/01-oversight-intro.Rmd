...
$$Pr(Comment_i) \sim \beta_0 + \beta_1 text{Number of comments} + \eta X_i 
(\#eq:oversight1)$$
...
The probability that a legislator will comment, given their position $p$ on a proposed rule ($Pr(Comment_i|p_i)$), is modeled in equasion \@ref(eq:oversight2). Hypothesis \@ref(hyp:beacon) implies that $\beta_1$ is positive and
Hypothesis \@ref(hyp:warning) implies that $\beta_2$ is negative.


$$Pr(Comment_i|p_i) \sim \beta_0 + \beta_1 \text{Comments supporting } p_i + \beta_2 \text{Comments opposing } p_i + \eta X_i
(\#eq:oversight2)$$
