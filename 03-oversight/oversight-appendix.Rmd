...
```{r oversighht-data, fig.height = 6}
# load from book repo 
...
load(here::here("data", "members.rda") %>% str_replace("dissertation", "augmentCongress"))
...
### FOIA Data
```{r oversighht-data-party-FOIA, fig.height = 2.5}
# by party
d %>% 
  filter(!is.na(FROM),
...
## All Data
```{r oversighht-data-party, fig.height = 2.5}
# by party
d %>% 
...
```{r oversighht-data-agency, fig.height = 18, fig.width=7.5}
d %>% 
  filter(!POLICY_EVENT %in%  c("Disaster", "Decision", "Meeting", "Information", "Lawsuit")) %>% 
  drop_na(agency) %>% 
  add_count(agency) %>% 
...
```{r oversighht-data-agency-top, fig.height = 5, fig.width=7.5}
# TOP 10 agencies
d %>% 
  filter(!POLICY_EVENT %in%  c("Disaster", "Decision", "Meeting", "Information", "Lawsuit")) %>% 
  drop_na(agency) %>% 
...
```{r oversighht-data-agency-top-top, fig.height = 2.5, fig.width=7.5}
# TOP 10 agencies
d %>% 
  filter(!POLICY_EVENT %in%  c("Disaster", "Decision", "Meeting", "Information", "Lawsuit")) %>% 
  drop_na(agency) %>% 
...
```{r oversighht-prepost, fig.height=2.5, cache=FALSE}
d %<>% 
...
```{r oversighht-drivers-prepost, fig.height=2.5, fig.width=7.5}
...
```{r oversighht-corporate-by-event}
d %>% 
