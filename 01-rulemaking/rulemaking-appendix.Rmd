...
ua %<>% mutate(Status = ifelse(is.na(WITHDRAWAL) | !str_detect(WITHDRAWAL, "-"), "Published", "Withdrawn"))

p <- ua %>% 
    count(Year, Status) %>%
    ggplot() + 
    aes(x = as.integer(Year), y = n, fill = Status) + 
    geom_col() +
    labs(#title = paste('"Significant"', unique(policy), 'Rulemaking Projects'),
      ...
      theme(panel.border = element_blank(),
          ...
          element_text(#size  =2,
                       ...
                       hjust = 1)) +#, vjust = .5)) + 
        scale_x_continuous(breaks = breaks) +
    scale_fill_viridis_d(option = "cividis", end = .6, direction = -1)

library(cowplot)
plot_grid(p, tile, align = "v", nrow = 2, rel_heights = c(2.2,1))
    ...
    labs(title = paste(unique(policy), 'Rulemaking'),
         subtitle = paste0("(", 
                           paste(x$PARENT_AGENCY %>%
                                   unique() %>% str_remove_all("^[0-9]*|[A-Z]*$"), sep = "and"),
                           ")"),
                x = "",
         y = "Number of Rules") +
      scale_x_continuous(breaks = breaks) +
...
```{r ua-Foreign Poicy, fig.height=3.2, cache=FALSE}
policy <- "Foreign Poicy"
...
```{r ua-Foriegn, fig.height=3.2, cache=FALSE}
policy <- "Foriegn"
...
```{r ua-Environment, fig.height=3.2, cache=FALSE}
policy <- "Environment"
