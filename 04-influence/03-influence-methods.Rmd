...
#   select(comment_url, success, docket_id, org_name, position, coalition_comment, coalition_type, org_type, number_of_comments_received) %>%
   mutate(number_of_comments_received = number_of_comments_received %>% as.numeric(),
          success = as.numeric(success)) #%>% 
#   full_join(mass_coded  %>% 
#               select(comment_url, success, docket_id, org_name, position, coalition_comment, coalition_type, org_type, number_of_comments_received))
```
...
This includes all proposed rules from 144 agencies that were open for comment on regulations.gov between 2005 and 2020, that received at least one comment from an organization, and that saw a final agency action between 2005 and 2020. 
There are over `r 50` million comments on this set of rules. 
...
Because my hypotheses are about the influence of organizations and coalitions, I collapse these data to one observation per organization or coalition per proposed rule for analysis. I then dentify the main substantive comment submitted by each organization's staff or lawyers, which are usually much longer than supporting comments like form letters. 
For hand-coding, I first select a random sample of proposed rules with a mass-comment campaign. I then selected all comments that were likely to be from organizations.^[Through an itterative process, I developed software and methods to select comments that were most likely to have been submitted by organizations rather than by individuals. For example, I include all comments submitted as file attachments rather than typed into the textbox.]
...
First, on a sample of rules, I hand-code lobbying success for each organzation or elected official, comparing the change between the draft and final rule to each organization's demands on a five-point scale from "mostly as requested" to "significantly different/opposite than requested." 
...
The average hand-coded success per organizational comment is `r mean(d$success, na.rm = T) %>% round()` (N = `r nrow(d)`). The average success for organizational comments with a mass comment campaign is `r mean(mass_coded$success, na.rm = T) %>% round(3)` (N = `r nrow(mass_coded)`).
...
The number and type(s) of organization(s) is an attribute of each coalition (e.g. a *business-led* coalition with *N* organizational members). Coalition *size* is the number of distinct commenting organizations in the coalition. For organizations lobbying alone, coalition *size* is 1. 

A coalition is *unopposed* when no opposing organizations comment. this is only for the hand-coded sample where we ahve coded the spatial position of each comment. 

I code a coalition as *business-led* if the majority of commenting organizations are for-profit businesses, or if upon investigation, I find it to be primarily led or sponsored by for-profit businesses.^[For more on how I identify types of organizations and coalitions, see \@ref(whymail-methods)]

### Examples of hand-coded lobbying success

**A rule with a public pressure campaigns: the 2015 Waters of the United States Rule:**
...
"] I also coded it as requesting speedy publication. These demands provide specific keywords and phrases for which to search in the draft and final rule text. The comparing the requeted policy outcomes to text of the final rule, I evaluate the extent to which NRDC got what it asked for.
