...
\@ref(fig:hist-coalition-comments) shows that most coalitions in these data include no *members of congress*, but some have as many as `r coalitions_coded$coalition_congress %>% max()`. The total number of *mass comments* is somewhat bimodal, reflecting the two random samples of rules from which these rules come. Most coalitions have did not mobilize a pressure campaign and thus have no mass comments, but a few have over a million. The models below use either a binary indicator for mass comments, the logged value, or the number rescaled as hundreds of thousands of comments. 
