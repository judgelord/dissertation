...
I now turn to a key variable omitted from the models in Table \@ref(tab:success), the level of attention and support from an agency's political principals---specifically, members of Congress, which may mediate the effect of public pressure campaigns. 
...
<!--
Model 2 shows a similar model, adding a fixed effect for the rulemaking docket (i.e., each policy on which an organization comments). Because I consider policies published by different administrations to be distinct, regardless of the docket id number used, the president and agency do not vary across dockets, and it no longer makes sense to include these variables. 
