...
d <- comments_coded %>% 
  dplyr::select(document_id, success, docket_id, org_name, position, coalition_comment, coalition_type, org_type, number_of_comments_received) 
...
I equasion \@ref(eq:oversight1) $y_{j}$ is a count of the number of legislator comments on a proposed rule $j$, $\beta_1$ is the effect of a one unit increase in the logged number of public comments on proposed rule $j$, and $\eta$ is a vector of coefficients on other factors ($X_{j}$) that may lead legislators to comment.

$$
y_{j}= \beta_0 + \beta_1 \text{log(Public comments)}_{j} + \eta X_{j} + \epsilon_{j} (\#eq:oversight)
$$
---
...
<!--gthe relative length of the (lead) organizations comment,--> 
...
Model 1: 
$$
y_ij = \beta_0 + \beta_1 log(comments_i) + \beta_2 length_i + \beta_3 unopposed_i + \beta_4 size_i + \beta_5 \text{Public Coalition}_i + \epsilon_i
$$
