...
\begin{equation}
Y_{ij} =  \alpha_i + \beta_1 \textbf{Pressure Campaign}_{i}) + \beta_{2-n} X_{ij} + \gamma_k + \delta_p + \epsilon_{i}
(\#eq:dnd)
\end{equation}
<!--+ {\beta}_5\text{Coalition Type}_{ij} +{\beta}_6\text{Coalition Type}_{ij}*\text{President}_{j} -->

Equation \@ref(eq:dnd) is similar to the model of coalition-level success except that  $y_{ij}$ is now the lobbying success of *organization* $i$ in coalition $j$ and $X_ij$ is now a vector of organization- and coalition-level controls.  Additionally, Equation \@ref(eq:dnd) adds  $\alpha_{i}$, a fixed effect for
