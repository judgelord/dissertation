...
#save(rules, file =  here("data", "rules_metadata.Rdata"))
  

dockets <- rules %<>% 
  distinct(docket_title, posted_date, docket_id, docket_type, number_of_comments_received, open) %>% 
...
From 2005 to  2020, agencies posted `r nrow(distinct(rules, docket_id))` rulemaking dockets to regulations.gov. and solicited public comments on `r nrow(distinct(rules %>% filter(open), docket_id))`. Only `r nrow(distinct(mass, docket_id))` of these rulemaking dockets received more than 100 comments, but this small share of rules garnered `r percent(sum(mass$number_of_comments_received)/sum(rules$number_of_comments_received), accuracy = .01)` (`r sum(mass$number_of_comments_received)`) of the comments. The top 10 rulemaking dockets account for `r percent(sum(top10$comments)/sum(rules$number_of_comments_received), accuracy = .01)` (`r sum(top10$comments)`), of the comments. Nearly all of these are form letters.

```{r}

n <- dockets %>% ungroup() %>%  filter(open) %>% count(number_of_comments_received, sort = T)
```



Figure
\@ref(fig:comments-density) shows the relative number of comments received on a sample of rules matched to data from the Office of Information and Regulatory Affairs (OIRA). OIRA mostly reports on "significant" rules and classifies them as "major" and "non-major" rules. The top part of Figure \@ref(fig:comments-density) shows the distribution of proposed rules over the number of comments they received (the x-axis). The lower part of Figure \@ref(fig:comments-density) shows a scatterplot of the number of rules with a given number of comments. The key takeaway is that there is a great deal of diversity in commenting, with most rules receiving very few comments. Indeed, overall (including non-significant proposed rules), the majority (`r n$n[1]` of `r nrow(dockets %>% filter(open))`) of proposed rules that are open for public comment received no comments.  Nearly 8,000 of the 20,000  (37%) of the non-major rules reviewed by OIRA received no comments. Even a quarter of "major" receive no comments. The modal number of comments for all types of rules, including major and economically significant rules, is less than ten. 

```{r comments-density, out.width = "99%", fig.cap = "Comments Posted to Regulations.gov on Rules Reviewed by OIRA 2005-2020"}
...
dockets %>%  
  filter(open) %>% 
    ...
    ungroup() %>% #count(docket_id, sort= T)
    arrange(-comments) %>% # filter(comments < 1)
    head(10) %>%
    mutate(docket_title = docket_title %>% #str_replace_all(";|:|–|--", ";\n") %>% 
               str_sub(0,50)) %>%
    mutate(docket_title = ifelse(nchar(docket_title) == 50, str_c(docket_title, "..."), docket_title) ) %>%# pull(docket_title)
    # ggplot() +
...
Figure \@ref(fig:comments-per-year) shows an exponential increase in the number of rules receiving a large number of comments from 2005 to 2013. Note that comments per rule (the y-axis) are on a
