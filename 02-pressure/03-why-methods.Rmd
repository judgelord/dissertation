...
From 2005 to  2020, agencies posted `r nrow(distinct(d, docket_id))` rulemaking dockets to regulations.gov and solicited public comments on `r nrow(distinct(d %>% filter(open), docket_id))`. Only `r nrow(distinct(mass, docket_id))` of these rulemaking dockets were targeted by one or more public pressure campaigns, but this small share of rules garnered `r round(sum(mass$number_of_comments_received)/sum(d$number_of_comments_received), 4)*100` percent (`r sum(mass$number_of_comments_received)`) of all comments. Nearly all of these comments are form letters. The top 10 rulemaking dockets account for `r round(sum(topdockets$docket_comments)/sum(d$number_of_comments_received), 4)*100` percent (`r sum(topdockets$docket_comments)`), of all comments in agency rulemaking. Again, nearly all of these are form letters.
...
This plot shows an increase in the number of rules receiving a large number of comments from 2005 to 2020. Note that comments per rule (the y-axis) are on a
