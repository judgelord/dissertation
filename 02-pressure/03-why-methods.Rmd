...
To assess my hypotheses about which groups should mobilize public participation in bureaucratic policymaking, I collected a corpus of approximately 80 million public comments via the
regulations.gov API. About 50 million of these comments were on proposed
rules (over 16,000 proposed rules from 144 agencies from 2005 to 2020). Of these, public pressure campaigns targeted approximately 2000 rules from 46 agencies.
...
```{r comments-per-year, fig.cap = "Comments per Proposed Rule 2005-2020 (log scale)", out.width = NULL, out.height = "25%", fig.show = "hold"}
...
Specifially, I assess the extent to which public comment are mobilized by pressure campaigns, which groups are behind these campaigns, which campaings are more successful in mobilizing, and which campaigns go unpposed. 



<!-- step 1-->
To identify whether a given comment is mobilize by a pressure campaign, I use several strategies.
 I first use textual similarity to identify
clusters of similar comments, reflecting formal and informal coalitions.
Comments with identical text (if any) indicate a coordinated campaign. 

<!-- step 2 REWRITE-->
Second, I hand-code a large sample of comments. I link individual comments to the more sophisticated
lobbying efforts they support.
...
### To What Extent is Public Participation Mediated by Groups?

```{r comments-per-year, fig.cap = "Public Comments 2005-2020", out.width = NULL, out.height = "25%", fig.show = "hold"}
