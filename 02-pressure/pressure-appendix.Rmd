...
## Comments
Figure
\@ref(fig:comment-from) shows the sources of incoming comments 
...
org_comments %<>% mutate(org_name = organization %>% 
                           str_rm_all("\\(.*| Action Fund|^The |of the United States| - .*|:.*|Mass Comment Campaign of the |/.*| USA$") %>%
                           str_replace(".*Sierra.*", "Sierra Club")%>% 
                           str_replace("NRDC.*|Natural Resources Defense Council.*", "Natural Resources Defense Council")%>% 
                           str_replace(".*Center For Biological Diversity.*", "Center For Biological Diversity")%>% 
                           ...
                           str_replace("MOM'S RISING|MOMSRIISNG", "MOMS RISING")%>%
                           str_replace("Preventobesity.*", "American Heart Association")%>%
                           str_replace(".*AXCESS\\b.*", "AXCESS FINANCIAL")%>%
                           ...
                           str_squish() %>%
                           str_to_title(),
                         organization = ifelse(nchar(organization)<10, str_to_upper(organization), organization) )

# On non-rulemaking dockets 
#org_comments %>% filter(str_dct(organization, "Electric Cooperatives"))

save(here::here(org_comments, "data", "org_comments.Rdata"))
...
## Oganization types
Membership organization comments to look like this comment from the NorthEast Hook Fisherman's Association:
