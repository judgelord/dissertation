...
coalitions_coded$Coalition_Position %<>% str_to_title()
...
```{r rules-by-campaign, fig.height= 3, fig.width = 5, fig.cap= "Proposed Rules Open for Comment on Regulations.gov 2005-2020"}
#rules-by-campaign
d %>% 
  ungroup() %>% 
  ...
  labs(y = "Number of Proposed Rules", 
       ...
       caption = "",
       fill = "") + 
  facet_wrap("campaign", scale = "free_y", ncol = 3) +
  #facet_grid(campaign ~ Agency, scales = "free") +
  scale_x_continuous(breaks = seq(2006, 2020, by = 2)) +
  theme_minimal() + 
...
```{r rules-by-campaign-agency, fig.height= 5, fig.width = 7, fig.cap= "Proposed Rules Open for Comment on Regulations.gov 2005-2020"}
# faceted by agency
d %>% 
  ungroup() %>% 
  left_join(topagencies) %>% 
  ...
  mutate(campaign = ifelse(campaign, "Targeted by a\nPressure Campaign",
         ...
         Agency = Agency %>% replace_na("All other agencies")) %>% 
  ggplot(  ) +
  aes(x = year, fill = campaign) + 
  geom_bar(alpha = .7) + 
  labs(y = "Number of Proposed Rules", 
       x = "",
       caption = "",
       fill = "") + 
  facet_wrap("Agency", scale = "free_y", ncol = 3) +
  #facet_grid(campaign ~ Agency, scales = "free") +
  scale_x_continuous(breaks = seq(2006, 2020, by = 2)) +
  theme_minimal() + 
  theme(panel.grid.major.x = element_blank(),
...
```{r campaign-type-frequency, fig.height=1.7, fig.width=4, cache=FALSE}
...
```{r campaign-type-scale, fig.height=1.7, fig.width=4, cache=FALSE}
#campaign-type-scale

coalitions_coded %>% 
  ungroup() %>% 
  drop_na(coalition_type, Coalition_Position) %>% 
  group_by(Coalition_Position, coalition_type, president) %>% 
  tally(coalition_comments, name = "coalition_comments") %>% 
  filter(president != "Bush") %>% 
  #mutate(epa = ifelse(agency == "EPA", "EPA", "All Other Agencies")) %>% 
  ggplot() +
...
coalitions_coded %>% 
  ...
  drop_na(coalition_type, Coalition_Position) %>% 
    mutate(epa = ifelse(agency == "EPA", "EPA", "All Other Agencies")) %>% 
  group_by(Coalition_Position, coalition_type, president, epa) %>% 
  tally(coalition_comments, name = "coalition_comments") %>% 
  filter(president != "Bush") %>% 
  ggplot() +
  aes(y = coalition_comments/1000000, 
