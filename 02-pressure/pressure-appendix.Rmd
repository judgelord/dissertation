...
# second step
comments_mass <- comments_min %>% filter(number_of_comments_received > 99 | docket_id %in% mass$docket_id)
#FIXME
...
```{r}
n <- d %>% ungroup() %>%  filter(open) %>% count(number_of_comments_received, sort = T)
```
...
annotate("text", x=4, y= 1.5, label=TeX("Comment asks for policy to move in the opposite direction, $p_1 < x_1$:"), parse=TRUE) +
  ...
  # NA
annotate("text", x=4, y= -.5, label=TeX("Comment asks for policy much closer to current policy, $x_1 > p_3 > x_2$:"), parse=TRUE) +
  geom_point(x = 2.5, y = -1, aes(color = label)) +
  geom_segment(aes(color = label), y = -1, yend = -1, x = 0, xend = 8, arrow = arrow(ends = "both", type = "closed", length = unit(0.1, "cm"))) +
   geom_line(aes(color = label), y = -1, yend = -1,) +
      # 3
annotate("text", x=4, y= -1.5, label=TeX("Comment asks for policy slightly closer to current policy, $x_1 > p_3 > x_2$:"), parse=TRUE) +
