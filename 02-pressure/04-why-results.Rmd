...
`r round(sum(top10$comments)/sum(d$number_of_comments_received), 4)*100` percent of comments on proposed rules posted to regulations.gov (`r sum(top10$comments)`).
All of these top ten organzations have lawyers on staff that engage in sophisticated lobbying and all ten lobbying together in the same coalitions. Nine are closely-aligned environmental groups. Earthjustice began as the Sierra Club Legal Defense Fund. Eight of these nine organizations (all but the Center for Biological Diversity) are members of the Partnership Project, a 501c3 nonprofit founded by 20 leading environmetnal groups with the aim of " creating a sum of citizen participation and advocacy greater than they could generate acting apart" [@Saveourenvironment]. CREDO Action is a generic progressive group often mobilzed to amplify progressive public interest campaings.

The top 100 organizations mobilized
`r round(sum(top100$comments)/sum(d$number_of_comments_received), 4)*100` percent of comments on proposed rules posted to regulations.gov (`r sum(top100$comments)`). Each mobilized between `r top100$comments[100]` and `r top100$comments[1]` comments.
...
coalitions_coded$coalition_type %<>% str_to_title()

# coalitions by type 
coalition_type_campaigns <- coalitions_coded %>%
  ...
  mutate(Total = `Mass Comment Campaign` + `No Mass Comment Campaign`)

total = sum(coalition_type_campaigns$Total)
total_public <- coalition_type_campaigns$Total[2]
total_private <- coalition_type_campaigns$Total[1]

percent_public <- round(( total_public/total )*100)
percent_private <- round(( total_private/total )*100)

# mass total by coalition type 
coalition_type_totals <- coalitions_coded %>% 
  filter(campaign_) %>% 
...
average_comments_public <- coalition_type_totals$`Average Comments`[2]
average_comments_private <- coalition_type_totals$`Average Comments`[1]

# mass total where  both coalition types lobbied
both_coalition_type_totals <- coalitions_coded %>% 
  group_by(docket_id, president) %>% 
  mutate(coalition_types = coalition_type %>% unique() %>% str_c(collapse = ";")) %>% 
  ungroup() %>%
  filter(campaign_, str_detect( coalition_comment_types,";")) %>% 
  group_by(coalition_type) %>% 
  summarise(`Total Comments` = sum(coalition_comments),
            `Average Comments` = mean(comments) %>% round) %>% drop_na(coalition_type) %>%
  rename(`Coalition Type` = coalition_type) 

average_comments_public_both <- both_coalition_type_totals$`Average Comments`[2]
average_comments_private_both <- both_coalition_type_totals$`Average Comments`[1]
```


In line with Hypothesis \@ref(hyp:public), `r percent_public` percent of public pressure campaigns are led by public interest coalitions, with only `r percent_private` percent led by private interest coalitions. While public interest groups lobbied slightly more often in the sample selected for hand coding (because half the sample was weighted to capture rules that received more comments), private interest coalitions are more common in the broader sample.  In contrast, Table \@ref(tab:coalition-type-campaigns) shows that mass comment campaigns were twice as likely to be led by public interest groups. 


```{r coalition-type-campaigns}
coalition_type_campaigns %>% 
...
In line with Hypothesis \@ref(hyp:public-success), public interest mass comment campaigns, on average, garner a larger response. The average number of comments for a public interest campaign is `r average_comments_public`. In contrast, the average number of comments for a private interest campaign is `r average_comments_private`. On rules where both public and private interest coalitions ran campaigns, the average response to the public interest campaign was `r average_comments_public_both`. Private interest campaigns averaged `r average_comments_private_both` comments.


```{r coalition-type-totals}
coalition_type_totals %>% 
...
types <- coalitions_coded %>%
  filter(coalition_, campaign_) %>%
  group_by(docket_id, president) %>% 
  mutate(coalition_types = coalition_type %>% unique() %>% str_c(collapse = ";")) %>% 
  ungroup() %>%
  mutate(both = str_detect( coalition_comment_types,";")) 
...
Just ten rulemaking dockets account for `r round(sum(topdockets$docket_comments)/sum(d$number_of_comments_received), 2)*100` percent of comments (`r sum(topdockets$docket_comments)`). Figure
