...
From `r min(rules$year, na.rm = T)` to  `r max(rules$year, na.rm = T)`, agencies posted `r nrow(distinct(rules, docket_id))` rulemaking dockets to regulations.gov. and solicited public comments on `r nrow(distinct(rules %>% filter(open), docket_id))`,  receivign `r sum(rules$number_of_comments_received)`. Only `r nrow(distinct(mass, docket_id))` rulemaking dockets received more than 100 comments, but these account for `r percent(sum(mass$number_of_comments_received)/sum(rules$number_of_comments_received), accuracy = .01)`, or `r sum(mass$number_of_comments_received)`, of the comments.
...
```{r comments-density, out.width = "49%", fig.cap = "Comments on Draft Rules Posted to Regulations.gov 2006-2018"}

knitr::include_graphics(here::here("Figs", "major-comments-density-1.png"))
```

Figure
\@ref(fig:comment-density) shows the sources of incoming comments 
```{r, comments-from, out.width = "49%", fig.cap = "Comments on Draft Rules Posted to Regulations.gov 2006-2018"}
...
Figure
\@ref(fig:comments-mass) shows all comments posted on
...
```{r comments-mass, out.width = "49%", fig.cap = "Comments on Draft Rules Posted to Regulations.gov 2006-2018"}
