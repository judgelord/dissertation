...
abstract: "Do public pressure campaigns affect policies made by unelected bureaucrats? In this article, I develop several measures of lobbying success and corresponding tests of whether public pressure campaings increase lobbying success. I then theorize mechanisms by which mass public engagment may affect policy. Each mechanism involves a distinct type of information revealed to decisionmakers."
---
```{r setup}
source("setup.R")
```
...
# Methods
The simplist way to falsify the hypothes that mass engagment increases lobbying success is to estabilish no relationship. Thus, before examining evidence for or against different pathways to influence, the first step is to assess the raw correlation. It is possible that bureaucrats anticipate public pressure campaigns when writing draft rules, muting effects at the final rule stage of the policy process, but this is a different hypothesis that would require different measures.  
...
For the State of Oregon's coalition, the dependent variable, *coalitions success* is 2, *coalition size* is 6, *business coalition* is 0, *comment length* is 5/85, `r round(5/85, 2)`, and *log mass comments*, is log(1), `r log(1)`.
...
To illustrate my planned analysis, I simulate data for each of the variables described above. 

**Dependent variables:** *Coalition success* is drawn from a descrete distribution {-2, -1, 0, 1, 2}. 

**Explainatory variables:** *Coalition size* (a count) is drawn from a Poisson distribution. *Business colation* is binomial. In reality, business coalitions are more common than non-bussness coalitions, but here I estimate a ballenced sample. I set rule pages constant at 85 and draw comment lengths from a Poisson distribution. While in reality, less than one precent of coalitions lobbying in rulemaking opt for a mass-comment campaign, I aim to gather a ballenced sample, so half of the simulated data are assumed to have no mass comment campaign and the other half have a number of comments drawn from a Zero-Truncated Poisson distribution. 
...
Unsurprisingly a model predicting lobbying success as a combination of coalition size, business coalition, comment length, and mass comments yields no significant results. 
```{r model_success}
